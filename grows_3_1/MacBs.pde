...
class MBasic extends Macro_Bloc { 
  ...
  void rebuild() {
    if (!rebuilding) {
      ...
      sValueBloc _bloc = copy_bloc(value_bloc, mmain().inter.data);
      clear();
      sValueBloc v_bloc = copy_bloc(_bloc, sheet.value_bloc);
...
class MSheetObj extends Macro_Bloc { 
  ...
  MSheetObj(Macro_Sheet _sheet, sValueBloc _bloc) { 
    ...
    get_run = new Runnable() { public void run() {
      out.send(newPacketSheet(sheet));
    }};
...
class MData extends Macro_Bloc {
  ...
  void setValue(sObj v) {
    oval = v;
    val_run = new Runnable() { public void run() { out.send(oval.asPacket()); }};
...
class MSheetView extends Macro_Bloc { 
  ...
  MSheetView(Macro_Sheet _sheet, sValueBloc _bloc) { 
    ...
    goto_run = new Runnable() { public void run() {
    ...
    in = addInput(0, "goto", new Runnable() { public void run() { 
      if (in.getLastPacket().isBang()) goto_run.run(); }});
  }
  MSheetView clear() {
    super.clear(); 
    return this; }
  MSheetView toLayerTop() {
    super.toLayerTop(); 
    return this; }
}
