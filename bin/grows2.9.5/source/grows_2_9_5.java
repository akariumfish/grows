...
class sInterface {
    ...
    if (files_panel == null) {
      ...
      files_panel.getShelf()
        ...
        .addDrawer(0.75f)
          ...
          .addLinkedModel("Field-SS3").setLinkedValue(savepath_value).setPX(ref_size*5).getShelf()
        .addSeparator(0.25f)
        .addDrawer(1)
      ...
      files_panel.addShelf()
        ...
        .addDrawer(10, 1)
          .addCtrlModel("Button_Small_Text_Outline-S3-P1", "delete file bloc")
            ...
            .getDrawer()
          .addCtrlModel("Button_Small_Text_Outline-S3-P2", "delete bloc")
            .setRunnable(new Runnable() { public void run() { 
  ...
  public void file_explorer_save() {
    if (file_explorer != null && !savepath_value.get().equals("default.sdata")) {
      file_savebloc.clear();
  ...
  */
  ...
  public void setup_load() {
    //nPopUp pop = new nPopUp(screen_gui, taskpanel, "LOADING...", true);
...
class MRamp extends Macro_Bloc { 
  ...
  sInt val_length;
  nLinkedWidget len_field, str_field, end_field;
  Runnable reset_run;
  MRamp(Macro_Sheet _sheet, sValueBloc _bloc) { 
    ...
    val_length = newInt("val_leng", "leng", 1);
    val_strt = newFlt("val_strt", "str", 1);
    val_end = newFlt("val_end", "end", 1);
    
    reset_run = new Runnable() { public void run() { got_reset(); } };
    
    val_length.addEventChange(reset_run);
    ...
    in_tick = addInput(0, "tick").addEventReceive(new Runnable() { public void run() { 
      if (in_tick.getLastPacket() != null && in_tick.getLastPacket().isBang()) { got_tick(); } } });
    in_start = addInput(0, "start").addEventReceive(new Runnable() { public void run() { 
      if (in_start.getLastPacket() != null && in_start.getLastPacket().isBang()) { got_start(); } } });
    in_stop = addInput(0, "stop").addEventReceive(new Runnable() { public void run() { 
    ...
    len_field = addEmptyS(1).addLinkedModel("MC_Element_SField").setLinkedValue(val_length);
    str_field = addEmptyS(1).addLinkedModel("MC_Element_SField").setLinkedValue(val_strt);
    end_field = addEmptyS(1).addLinkedModel("MC_Element_SField").setLinkedValue(val_end);
    str_field.setInfo("start value");
    end_field.setInfo("end value");
    len_field.setInfo("length in tick");
  ...
  public float getval() {
    float v = 0;
    if (valLIN.get()) v = val_strt.get() + (PApplet.parseFloat(count) / PApplet.parseFloat(val_length.get())) * (val_end.get() - val_strt.get());
    else v = val_strt.get() + (0.5f + cos(PI * (PApplet.parseFloat(count) / PApplet.parseFloat(val_length.get()))) / 2 ) * (val_end.get() - val_strt.get());
    return v;
  }
  int count = 0;
...
class MData extends Macro_Bloc {
  ...
  boolean btmp; int itmp; float ftmp; String stmp = ""; PVector vtmp = new PVector();
  Macro_Connexion in, out;
  sStr val_cible; 
  sValue cible;
  nLinkedWidget ref_field; 
...
class MVar extends Macro_Bloc {
  ...
  sBoo bval; sInt ival; sFlt fval; sVec vval;
  sStr val_type;
...
Macro_Abstract(Macro_Sheet _sheet, String ty, String n, sValueBloc _bloc) {
  ...
  public void find_place() {
    ...
    while (!found) {
      ...
      adding_v++; 
      if (adding_v == 4) { 
        adding_v = 0; setPosition(grabber.getLocalX() - ref_size * 9, grabber.getLocalY() + ref_size * 6); }
      boolean col = false;
      float phf = 0.5f;
...
class Macro_Sheet extends Macro_Abstract {
  ...
  */
  ...
  MVar better saving
    currently saved as text, only bool n float work
    use an sval by type + sstr to remember type
    
  link center positionning and visibility
  
  macro couleur packetcol <> scol <> int rgb
  
  develop cursor
    custom setlinkedPosition(svec)
           setlinkedDir(svec)
           setlinkedShow(sboo)
    dif shape / look
  ...
  priority system
    add arrow n number with losenge horizontal on top right
    
  make manually added macro bloc to pop onscreen, not at origin
  ...
  enregistrement de different POV camera entre lesquelon peut basquler
  ...
  gameplay thinking :
    ...
    we will need to group shapes to limit object nb
...
Macro_Sheet(Macro_Sheet p, String n, sValueBloc _bloc) { 
  ...
  public void addCopyofBloc(sValueBloc bloc) {
    if (!bloc.ref.equals("settings")) {
      ...
      if (a != null) new_ref = new_ref + OBJ_TOKEN + bloc.base_ref+INFO_TOKEN+nbloc.ref;
...
class Macro_Main extends Macro_Sheet {
  ...
  public void new_tmpl() {
    if (selected_macro.size() > 0) {
      ...
      String nn = "group_"+new_temp_name.get();
      int t = 0;
      while (saved_template.getBloc(nn) != null) { nn = "group_"+new_temp_name.get() + "_" + t; t++; }
      ...
      String nn = "sheet_"+new_temp_name.get();
      int t = 0;
      while (saved_template.getBloc(nn) != null) { nn = "sheet_"+new_temp_name.get() + "_" + t; t++; }
      sValueBloc bloc = saved_template.newBloc(nn);
  ...
  public void copy_to_tmpl() {
    if (selected_macro.size() > 0) {
      if (pastebin != null) pastebin.clear();
      String nn = "copy_of_group_of_"+selected_macro.size();
      while (saved_template.getBloc(nn) != null) nn += "_";
      sValueBloc bloc = saved_template.newBloc(nn);
      for (Macro_Abstract m : selected_macro) copy_bloc(m.value_bloc, bloc);
      pastebin = saved_template.last_created_bloc;
  ...
  */
  ...
  sStr new_temp_name; sRun del_select_run;
  //sInt val_scale;
  sInterface inter;
...
class nExplorer extends nDrawer {
  ...
  public nDrawer toLayerTop() { super.toLayerTop(); myshelf.toLayerTop(); return this; }
...
//class nPopUp extends nWindowPanel {
  
//  nWidget info;
//  boolean autoclose = false;
//  nPopUp(nGUI _g, nTaskPanel _task, String t, boolean ac) { 
//    super(_g, _task, t); 
//    autoclose = ac;
//    info = getShelf().addSeparator(0.25)
//      .addDrawer(10.25, 1)
//        .addModel("Label-S4", t).setTextAlignment(LEFT, CENTER);
...
//          .setRunnable(new Runnable() { public void run() { clear(); } }).getShelf();
//    }
//  }
//  nWindowPanel clear() { 
//    super.clear(); return this; }
