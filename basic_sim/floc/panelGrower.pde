...
void init_panel_grower() {
  ...
  build_line_factor(group_grow, "CHASE", " ", POURSUITE, 10, 70, 0);
  build_line_factor(group_grow, "FOLLOW", " ", FOLLOW, 10, 120, 1);
...
void event_panel_grower(int id, int line, float modifier) {
  ...
  // apply modifier
  if (line == 0) { POURSUITE *= modifier; update_textlabel("CHASE", " ", POURSUITE); }
  if (line == 1) { FOLLOW *= modifier; update_textlabel("FOLLOW", " ", FOLLOW); }
