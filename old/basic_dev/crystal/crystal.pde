...
// to add dans le modele ! et ajoute un bp. Raccourcie : 'w'
...
void draw() {//executé once by frame
  ...
  if (turn_limit < counter) pause = true;
  ...
  //draw_on_camera:
  drawAll(); //execution des draw de tout les objet actif dans baselist
  
  //pointeur local sourie
  stroke(255, 0, 0);
  ellipse(mouse_pos.x, mouse_pos.y, 5, 5);
...
void cam_input_update() {
  ...
  mouse_pos.x = - cam_pos.x / cam_scale + ((mouseX - ((width - PANEL_WIDTH) / 2.0)) / cam_scale);
  mouse_pos.y = - cam_pos.y / cam_scale + ((mouseY - (height / 2.0)) / cam_scale);
}
