...
class Camera {
  ...
  PVector getCamMouse() { return screen_to_cam(new PVector(mouseX, mouseY)); }
...
void mouseWheel(MouseEvent event) { kb.mouseWheelEvent(event); }  
void keyPressed() { kb.keyPressedEvent(); }  
void keyReleased() { kb.keyReleasedEvent(); }
void mousePressed() { kb.mousePressedEvent(); }
void mouseReleased() { kb.mouseReleasedEvent(); }
void mouseDragged() { kb.mouseDraggedEvent(); }
...
public class sInput {
  ...
  void mouseDraggedEvent() { mouseMove = true; }
  
  void mouseMovedEvent() { mouseMove = true; }
}
